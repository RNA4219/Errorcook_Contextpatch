...
import { parseFailureOutput } from '../parsers/failure_parser';
...
/**
 * Process raw CI tool output through parser and analysis workflow
 */
export async function processRawCIOutput(
  toolName: string,
  rawOutput: any[]
): Promise<{
  triageResults: TriageResult[];
  ...
  // Parse the raw CI output into standardized FailureItems
  const failures = parseFailureOutput(toolName, rawOutput);
  
  // Process through the analysis workflow
  const workflow = new FailureAnalysisWorkflow();
  return workflow.processFailures(failures);
}